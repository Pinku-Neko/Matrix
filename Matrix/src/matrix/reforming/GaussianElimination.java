...
final class GaussianElimination {
	public static final <E extends Field<E>> void gaussianElimination(final Matrix<E> thisMatrix) {
		...
		for (int columnCount = 0; columnCount < thisMatrix.getLength(); columnCount++) {
			for (int currentEchelonRow = amountEchelon; currentEchelonRow < thisMatrix
					.getHeight(); currentEchelonRow++) {
	 ...
	 */
	private static final <E extends Field<E>> void cleanEchelonColumn(final Matrix<E> thisMatrix,
			final int actualCurrentEchelonIndex, final int currentEchelonRowIndex) {
		final Integer currentEchelonColumnPosition = thisMatrix.getMapEchelonColumn().get(actualCurrentEchelonIndex);
