...
/**
 * this class contains all available interactions between rows in the matrix,
 * such as exchanging positions, subtraction etc.
 * 
 * @author Pinku_Neko
 *
 */
final class RowsInteraction {
	 ...
	 * 
	 * postcondition: if index < 0 || index > thisMatrix.getHeight() throws
	 ...
	 * 
	 * precondition: matrix is inited, indexOfOneRow exists in matrix,
	 ...
	 */
	static final <E extends Field<E>> void exchangeRows(final Matrix<E> thisMatrix, final int indexOfOneRow,
	 ...
	 * 
	 * precondition: matrix is inited, rowIndex exists in matrix, columnPosition
	 * exists in matrix
	 * 
	 * clean the other rows in the matrix except the chosen row on the specified
	 * column. this is done by subtracting other rows by the chosen row multiplied
	 * by the value the row on this column has.
	 * 
	 * for example: clean row {2, 4, -2} by choosing {1, 1, 1} will result {1, 3,
	 * -3}
	 * 
	 * @param <E>            the field this matrix is using
	 * @param thisMatrix     the matrix
	 ...
	 */
	static final <E extends Field<E>> void cleanOtherRows(final Matrix<E> thisMatrix, final int rowIndex,
		...
		for (int i = 0; i < thisMatrix.getHeight(); i++) {
			...
			if (i != rowIndex) {
				subtractRow(thisMatrix, i,
						RowTransformation.multiplyRow(thisMatrix.getMatrix().elementAt(rowIndex), factor));
			}
		}
	 ...
	 * 
	 * precondition: matrix is inited, indexOfRowToBesubtracted exists in matrix
	 * 
	 * postcondition: all elements on the row will be subtracted by corresponding
	 * elements on another row
	 * 
	 * @param <E>                      the field this matrix is using
	 * @param thisMatrix               the matrix
	 ...
	 */
	private static final <E extends Field<E>> void subtractRow(final Matrix<E> thisMatrix,
			final int indexOfRowToBeSubtracted, final Vector<E> rowToSubtract) {
		isMatrixInited(thisMatrix);
